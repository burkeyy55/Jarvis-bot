...
const reflections = require('../data/reflections');
...
async function buildMorningMessage() {
  ...
  if (weather) {
    ...
    msg += `${weather.condition}, ${weather.temp}°C`;
    if (weather.rainChance > 20) {
      msg += ` · ${weather.rainChance}% chance of rain — you may want to plan accordingly.`;
    }
    msg += '\n\n';
  }
  ...
  if (sol) {
    ...
    msg += `${arrow} *Solana*\n`;
    msg += `$${sol.price} USD  ·  ${sign}${sol.change}% (24h)\n\n`;
  }
  ...
  msg += `Shall we get the day started, sir?\n\nUse /tasks to set your top 3 priorities for today.`;

  return msg;
}

async function buildEveningDebrief(chatId) {
  ...
  // Habits summary
  msg += `📊 *Habits Logged Today*\n`;
  if (habits.length === 0) {
    msg += `Nothing logged today, sir.\n`;
  } else {
    const logged = [...new Set(habits.map(h => h.habit))];
    logged.forEach(h => { msg += `✅ ${capitalize(h)}\n`; });
  }

  msg += `\n`;

  // Gratitude
  msg += `🙏 *One thing you're grateful for today?*\n`;
  msg += `Reply with /gratitude [your answer]\n\n`;
...
module.exports = {
  ...
  buildEveningDebrief,
  buildTaskNudge,
  buildWeeklyStrengthReport,
  buildWeeklyGoalCheckin,
  getRandomQuote,
  getRandomReflection,
  capitalize
};
